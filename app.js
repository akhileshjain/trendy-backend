...
app.get("/api/getLatestCashOrderNo", (req, res, next) => {
  CashOrder.findOne()
    .select("cashOrderNumber")
    .sort("-cashOrderNumber")
    .then((data) => {
      res.status(200).json({
        data: data,
      });
...
app.get("/api/bill", (req, res, next) => {
  ...
  if (orderId) {
    Bill.findOne({ challanNumber: orderId })
      .then((data) => {
        if (data) {
          res.status(200).json({
...
app.get("/api/cashorder", (req, res, next) => {
  let orderId = req.query.orderId;
  if (orderId) {
    CashOrder.findOne({ cashOrderNumber: orderId })
      .then((data) => {
        if (data) {
          res.status(200).json({
...
app.post("/api/deleteBill", (req, res, next) => {
  Bill.deleteOne({ challanNumber: req.body.challanNumber })
    .then((data) => {
      if (data) {
        res.status(201).json({
...
app.post("/api/deleteCashOrder", (req, res, next) => {
  CashOrder.deleteOne({ cashOrderNumber: req.body.cashOrderNumber })
    .then((data) => {
      if (data) {
        res.status(201).json({
